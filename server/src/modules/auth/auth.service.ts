...
export const verifyOTP = async (payload: verifyOTPProps) => {
  ...
  if (!isOtpValid) {
    ...
    if (updatedSignupRequest.otpAttempts >= 3) {
      await registerUserRepository.invalidateOtp(existsSignupRequest.id);
...
export const createNewAccount = async (payload: createNewAccountProps) => {
  ...
  const passwordHash = await bcrypt.hash(password, 10);
  const baseSlug = slugify(existsSignupRequest.companyName, {
    lower: true,
    strict: true,
  });
  const slug = `${baseSlug}-${crypto.randomBytes(3).toString('hex')}`;
  const result = await registerUserRepository.createAccountTransaction({
    existsSignupRequest,
    passwordHash,
    slug,
  });
