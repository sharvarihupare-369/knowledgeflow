...
const envSchema = yup.object({
  ...
  FRONTEND_URL: yup.string().matches(/^https?:\/\//, 'Must be a valid URL').default('http://localhost:3000'),
  APP_NAME: yup.string().default('KnowledgeFlow AI'),
  SECRET_KEY: yup.string().required('SECRET_KEY is required'),
  QDRANT_URL: yup.string().matches(/^https?:\/\//, 'Must be a valid URL').required('QDRANT_URL is required'),
  DATABASE_URL: yup.string().required('DATABASE_URL is required'),
  BACKEND_URL: yup.string().matches(/^https?:\/\//, 'Must be a valid URL').optional(),
});
